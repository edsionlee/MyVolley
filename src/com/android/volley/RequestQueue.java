 ...
 * RequestQueue类存在2个非常重要的PriorityBlockingQueue类型的成员字段mCacheQueue mNetworkQueue
 * ，该PriorityBlockingQueue为java1.5并发库提供的新类
 * 其中有几个重要的方法，比如take()为从队列中取得对象，如果队列不存在对象，将会被阻塞，直到队列中存在有对象，类似于Looper.loop()
 * 
 * 实例化一个request对象，调用RequestQueue.add(request),该request如果不允许被缓存，
 * 将会被添加至mNetworkQueue队列中，待多个NetworkDispatcher线程take()取出对象
 * 如果该request可以被缓存，该request将会被添加至mCacheQueue队列中，待mCacheDispatcher线程从mCacheQueue.
 * take()取出对象，
 * 如果该request在mCache中不存在匹配的缓存时，该request将会被移交添加至mNetworkQueue队列中，待网络访问完成后，
 * 将关键头信息添加至mCache缓存中去！
 */
public class RequestQueue {
	...
	/**
	 * Stops the cache and network dispatchers. 停止缓存处理器和网络处理器的任务
	 */
	public void stop() {
	...
	/**
	 * Adds a Request to the dispatch queue. 将请求添加到队列中
	 * 
	 * @param request
